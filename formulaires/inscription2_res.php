	...
	$erreurs = array();
 	...
 	if ($type_auteur == 'particulier'){
 		foreach ($valeurs  as $champs => $valeur) {
 			if ((lire_config('inscription2/'.$champs.'_obligatoire') == 'on' ) && (empty($valeur) OR (strlen(_request($champs)) == 0)) AND $champs!='statut_asso' AND $champs!='statut_juridique' AND $champs!='bio' AND $champs!='naissance') {
	...
	*/
    ...
    if (!$new){
		...
		if(!$traiter_plugin['ne_pas_confirmer_par_mail']){
			...
			if(_request('type_auteur')=='marque'){
					$arg_inser_liste = array(
					'id_auteur' => $id_auteur,
					'id_liste' => '4',
					'format' => 'html',
			...
			if(_request('type_auteur')=='ong' OR _request('type_auteur')=='lieu'){
					$arg_inser_liste = array(
					'id_auteur' => $id_auteur,
					'id_liste' => '5',
					'format' => 'html',
			...
			spip_query ( " INSERT INTO spip_articles (titre, id_rubrique, id_secteur, statut, lang,langue_choisie,accepter_forum,date) VALUES ("._q($titre).", "._q($id_rubrique).", "._q($id_rubrique).", "._q($statut).", "._q($lang).", "._q($langue_choisie).", "._q($accepter_forum).", "._q($date).") ");

				$article = spip_query( "SELECT * FROM spip_articles WHERE titre='$titre' AND id_rubrique='$id_rubrique' ");
				while ($name = mysql_fetch_array ($article)){
				$id_article = $name['id_article'];
...
.			$nom." vient de s'inscrire \n\nVoir son profile: ".$url_inscrit;
			...
			$envoyer_inscription_webmaster('inscription2','',$id_auteur);
		}
		   		if ($page){
